/* ===========================
   KFGC Media App — script.js (CLEAN & OPTIMIZED)
...
const THEME_KEY = 'kfgc_theme'; // light | dark
...
function showSlide(i) {
  ...
  qsa('.dot').forEach((d, n) => d.classList.toggle('active', n === slideIndex));
}
...
if (hero && bgImages.length) {
  setInterval(() => {
    hero.style.backgroundImage = `linear-gradient(rgba(0,0,0,.4),rgba(0,0,0,.4)), url('${bgImages[bgIndex]}')`;
...
window.shareApp = function () {
  const data = {
    ...
    text: 'Watch live services and sermons.',
    url: location.href
  };
  navigator.share ? navigator.share(data) : alert(data.url);
};

/* ---------- Prayer Form ---------- */
qs('#prayer-form')?.addEventListener('submit', (e) => {
  e.preventDefault();
  const v = qs('#member-request')?.value.trim();
  if (v) alert('Prayer request submitted.');
});

/* ---------- Font Size ---------- */
const SIZE_KEY = 'kfgc_fontsize';
function setFontSize(s) {
  document.body.classList.remove('font-sm','font-md','font-lg');
  document.body.classList.add('font-' + s);
  localStorage.setItem(SIZE_KEY, s);
}
setFontSize(localStorage.getItem(SIZE_KEY) || 'md');
qsa('.size-btn').forEach(b => b.onclick = () => setFontSize(b.dataset.size));

/* ---------- YouTube Player ---------- */
const ytFrame = qs('#youtubePlayer');
const channelId = 'UCbg2Da3jAPU13EZd6WyKjRw';

function loadLiveOrPlaylist() {
  if (!ytFrame) return;
  ytFrame.src = `https://www.youtube.com/embed/live_stream?channel=${channelId}`;
  setTimeout(() => {
    ytFrame.src = `https://www.youtube.com/embed?listType=playlist&list=UU${channelId.slice(2)}`;
  }, 5000);
}

setInterval(loadLiveOrPlaylist, 60000);

window.playSermon = function (id) {
  if (!ytFrame) return;
...
window.filterSermons = function () {
  const q = qs('#sermonSearch')?.value.toLowerCase() || '';
  const c = qs('#categoryFilter')?.value || 'all';
  qsa('.sermon-item').forEach(i => {
    const t = i.dataset.tags.toLowerCase();
    i.style.display = t.includes(q) && (c === 'all' || t.includes(c)) ? 'flex' : 'none';
  });
};

/* ---------- New Sermon Alert ---------- */
(function () {
  const latest = 'Y7wPDBjpPn8';
  if (localStorage.getItem('latestSermon') !== latest) {
    alert('📢 New sermon uploaded!');
    localStorage.setItem('latestSermon', latest);
  }
})();
